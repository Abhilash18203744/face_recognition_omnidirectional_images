...
def PCA_Train(data, n_components=100):
    print_info("number of components: %d"%n_components)
...
def data_pre(data):
    data = np.sqrt(data)
    print("data_sqrt {}".format(data))
    print("divider {}".format(np.repeat(np.sum(data, 1), data.shape[1]).reshape(data.shape[0], data.shape[1])))
